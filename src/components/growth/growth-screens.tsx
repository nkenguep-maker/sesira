...
function DemoBanner() {
  return (
    <section className="mt-6  border border-[var(--blue)] bg-[var(--blue-soft)] p-5" aria-label="Données de démonstration">
      <div className="flex items-start gap-3">
        <Sparkles className="mt-0.5 size-5 shrink-0 text-[var(--blue)]" />
        <div>
          <p className="text-xs font-semibold tracking-[0.15em] text-[var(--blue)]">DÉMO</p>
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {channels.map((channel) => (
        <span key={channel} className=" border border-[var(--border)] bg-[var(--panel-soft)] px-2.5 py-1 text-xs text-[var(--muted)]">
...
export function MarketingHomeScreen({
  ...
  return (
    ...
    >
      <section className="sesira-metric-grid mt-6 md:grid-cols-3" aria-label="Résumé Marketing">
...
function SummaryCard({ href, icon: Icon, label, value }: { href: string; icon: LucideIcon; label: string; value: number }) {
  void Icon;
  return (
    <Link href={href} className="group bg-[var(--surface)] p-5 transition hover:bg-[#f7f9fa] md:p-6">
      <div className="flex items-start justify-between gap-4">
        <span className="sesira-eyebrow">À PRÉPARER</span>
        <ArrowRight className="size-4 text-[var(--muted)] transition group-hover:translate-x-1 group-hover:text-[var(--blue)]" />
      </div>
      <p className="mt-7 [overflow-wrap:anywhere] font-[family-name:var(--font-display)] text-[2rem] font-semibold tracking-[-0.03em] tabular-nums">{value}</p>
...
function OrganizationKnowledgeCard({ knowledge }: { knowledge: OrganizationKnowledge }) {
  ...
  return (
    <article className=" border border-[var(--border)] bg-[var(--panel)] p-5 md:p-6">
      <div className="flex items-start gap-4">
        <Building2 className="mt-0.5 size-5 shrink-0 text-[var(--blue)]" />
        <div>
          <p className="text-xs font-semibold tracking-[0.15em] text-[var(--blue)]">VOTRE ENTREPRISE</p>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        {sections.map(([label, values]) => (
          <section key={label} className=" bg-[var(--panel-soft)] p-4">
        ...
        <section className=" border border-[var(--sand-line)] bg-[var(--sand)] p-4 sm:col-span-2">
          <h3 className="text-xs font-semibold uppercase tracking-[0.1em] text-[var(--sand-text)]">Certifications</h3>
...
function PreviewCard({ icon: Icon, eyebrow, title, description, href, linkLabel }: { icon: LucideIcon; eyebrow: string; title: string; description: string; href: string; linkLabel: string }) {
  return (
    <article className=" border border-[var(--border)] bg-[var(--panel)] p-5">
      <div className="flex items-center gap-2 text-[var(--blue)]"><Icon className="size-4" /><p className="sesira-eyebrow !text-[var(--blue)]">{eyebrow}</p></div>
      <h3 className="mt-3 font-medium">{title}</h3>
      <p className="mt-2 text-sm leading-6 text-[var(--muted)]">{description}</p>
      <Link href={href} className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-[var(--blue)]">{linkLabel}<ArrowRight className="size-4" /></Link>
    </article>
  );
}
...
export function MarketingIdeasScreen({ result }: { result: GrowthResult<GrowthIdea[]> }) {
  return (
    <GrowthPage eyebrow="MARKETING · IDÉES" title="Des sujets à transformer en contenus utiles." description="Chaque idée part d’une question client, d’une objection ou d’un sujet métier concret." source={result.source}>
      {result.data.length ? (
        <section className="mt-8 grid gap-px border border-[var(--line)] bg-[var(--line)] lg:grid-cols-2" aria-label="Idées à préparer">
          {result.data.map((idea) => (
            <article key={idea.id} className="bg-[var(--surface)] p-5 md:p-6">
              <div className="flex items-start justify-between gap-4">
                <span className="sesira-eyebrow !text-[var(--sand-text)]">IDÉE</span>
              ...
              <div className="mt-5  bg-[var(--panel-soft)] p-4">
...
export function MarketingContentScreen({ result }: { result: GrowthResult<GrowthContent[]> }) {
  return (
    <GrowthPage eyebrow="MARKETING · CONTENUS" title="Un parcours de validation lisible." description="De l’idée à la publication, chaque statut reste visible et la décision finale appartient à votre équipe." source={result.source}>
      <section className="mt-8 grid gap-px border border-[var(--line)] bg-[var(--line)] sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6" aria-label="Étapes du contenu">
        {Object.entries(CONTENT_STATUS_LABELS).map(([status, label], index) => (
          <div key={status} className="bg-[var(--surface)] p-4">
      ...
      {result.data.length ? (
        <section className="mt-6 grid gap-px border border-[var(--line)] bg-[var(--line)]" aria-label="Contenus marketing">
          {result.data.map((item) => (
            <article key={item.id} className="bg-[var(--surface)] p-5 md:p-6">
      ...
      {result.data.length ? (
        <>
          <section className="mt-8" aria-labelledby="calendar-title">
            <div className="flex items-center gap-2"><CalendarDays className="size-5 text-[var(--blue)]" /><h2 id="calendar-title" className="text-lg font-semibold">Prochaines dates</h2></div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {planned.map((item) => (
                <article key={item.id} className=" border border-[var(--blue)] bg-[var(--blue-soft)] p-5">
                  <p className="text-xs font-semibold uppercase tracking-[0.1em] text-[var(--blue)]">{dayFormatter.format(new Date(item.publicationAt))}</p>
          ...
          <section className="mt-10" aria-labelledby="publication-list-title">
            ...
            <div className="mt-4 overflow-hidden  border border-[var(--border)] bg-[var(--panel)]">
          ...
          <section className="mt-6 flex items-start gap-3  border border-[var(--sand-line)] bg-[var(--sand)] p-4">
            <ShieldCheck className="mt-0.5 size-4 shrink-0 text-[var(--sand-text)]" />

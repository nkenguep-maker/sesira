...
export default async function GrowthPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  if (campaignsResult.status === "ERROR" || leadsResult.status === "ERROR") {
    return <><PageHeader eyebrow="CROISSANCE" title="Vue croissance" description="Campagnes et leads avant leur entrée dans le pipeline commercial." /><section className="app-state-message"><strong>Croissance indisponible</strong><p>Une partie des données Growth n’est pas lisible. SESIRA ne calcule aucun taux à partir d’un jeu incomplet.</p></section></>;
  }
  ...
  return (
    <div className="sesira-page--growth">
      <PageHeader eyebrow="CROISSANCE" title="Vue croissance" description="Les leads qui demandent une décision et le contexte des campagnes qui les ont générés." />
      <ResultNotice result={params.result} />

      {campaigns.length || leads.length ? (
      ...
      <section className="workspace-boundary-note"><StatusPill>Contrôle humain</StatusPill><p>SESIRA peut organiser et suggérer. Il ne qualifie, ne disqualifie et ne convertit jamais un lead automatiquement.</p></section>
            ...
            {leads.map((lead) => (
              <article className="workspace-row" key={lead.id}>
                <div className="workspace-row-main">
                  <div className="workspace-row-heading"><div><span className="eyebrow">{sourceLabel(lead.source)}</span><h2>{lead.contactName}</h2></div><StatusPill tone={leadTone(lead.status)}>{leadLabel(lead.status)}</StatusPill></div>
                ...
                {lead.status === "NEW" ? (
                  <div className="workspace-row-actions">
                    <details className="sesira-action-drawer">
                      <summary>Décider</summary>
                      <div className="sesira-action-drawer-body">
                        <form action={qualifyLeadAction} className="workspace-inline-form compact"><input type="hidden" name="leadId" value={lead.id} /><label><span>Note facultative</span><input name="notes" maxLength={4000} placeholder="Pourquoi ce lead est pertinent" /></label><button type="submit" className="button primary small">Qualifier</button></form>
                        <form action={disqualifyLeadAction} className="workspace-inline-form compact"><input type="hidden" name="leadId" value={lead.id} /><label><span>Motif requis</span><input required name="reason" maxLength={500} placeholder="Doublon, hors cible…" /></label><button type="submit" className="button ghost small">Disqualifier</button></form>
                      </div>
                    </details>
                  </div>
                ) : null}
            ...
            {campaigns.map((campaign) => (
              <article className="workspace-row" key={campaign.id}>
                <div className="workspace-row-main">
                  ...
                  <div className="workspace-meta"><span><b>Début</b>{campaign.startAt ? formatDate(campaign.startAt) : "Non démarrée"}</span><span><b>Fin prévue</b>{campaign.endAt ? formatDate(campaign.endAt) : "Non renseignée"}</span><span><b>Budget indicatif</b>{campaign.budget === null ? "Non renseigné" : formatAmount(campaign.budget, campaign.currency)}</span><span><b>Référence externe</b>{campaign.externalRef ?? "Aucune"}</span></div>
                </div>
              </article>
            ))}
          </div>

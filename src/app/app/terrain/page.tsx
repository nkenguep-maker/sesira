...
import { addFieldNoteAction, arriveAtInterventionAction, resolveFieldConflictAction, startInterventionAction } from "./actions";
...
export default async function FieldPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <>
      <PageHeader eyebrow="TERRAIN" title="Ma journée" description="Les interventions qui vous sont assignées. Cette interface web enregistre les actions en ligne ; la synchronisation hors connexion complète reste une couche cliente distincte." />
      <ResultNotice result={params.result} />
      <section className="workspace-stat-strip"><div><strong>{interventions.length}</strong><span>Interventions</span></div><div><strong>{interventions.filter((row) => row.status === "IN_PROGRESS").length}</strong><span>En cours</span></div><div><strong>{interventions.filter((row) => row.status === "COMPLETED").length}</strong><span>Terminées</span></div><div><strong>{conflicts.length}</strong><span>Conflits de synchro</span></div></section>

      {conflicts.length ? <section className="workspace-section"><div className="workspace-section-heading"><div><span className="eyebrow">À VÉRIFIER</span><h2>Conflits de synchronisation</h2></div><StatusPill tone="warning">{conflicts.length}</StatusPill></div><div className="workspace-list compact-list">{conflicts.map((item) => <article className="workspace-row" key={item.artifactId}><div className="workspace-row-main"><div className="workspace-row-heading"><div><span className="eyebrow">{artifactLabel(item.artifactKind)}</span><h2>Élément terrain à arbitrer</h2></div><StatusPill tone="warning">Conflit</StatusPill></div><p className="workspace-description">{item.conflictReason ?? "Le serveur a conservé cet élément mais ne l’a pas appliqué automatiquement."}</p><div className="workspace-meta"><span><b>Capturé</b>{formatDateTime(item.capturedAt)}</span><span><b>Reçu</b>{formatDateTime(item.uploadedAt)}</span></div></div><div className="workspace-row-actions"><form action={resolveFieldConflictAction} className="workspace-inline-form compact"><input type="hidden" name="artifactId" value={item.artifactId} /><label><span>Décision</span><select name="resolution" defaultValue="SYNCED"><option value="SYNCED">Conserver comme synchronisé</option><option value="IGNORED">Ignorer cet élément</option></select></label><label><span>Note</span><input name="note" maxLength={1000} placeholder="Motif de la décision" /></label><button className="button primary small" type="submit">Enregistrer</button></form></div></article>)}</div></section> : null}

      {interventions.length ? <section className="workspace-list" aria-label="Interventions du jour">{interventions.map((row) => <article className="workspace-row" key={row.interventionId}><div className="workspace-row-main"><div className="workspace-row-heading"><div><span className="eyebrow">{row.scheduledAt ? formatTime(row.scheduledAt) : "Sans horaire"}</span><h2>{row.title}</h2></div><StatusPill tone={row.status === "IN_PROGRESS" ? "warning" : row.status === "COMPLETED" ? "good" : "neutral"}>{interventionLabel(row.status)}</StatusPill></div><div className="workspace-meta"><span><b>Client</b>{row.customerName ?? "Non renseigné"}</span><span><b>Adresse</b>{row.address || "Non renseignée"}</span><span><b>Durée prévue</b>{row.durationMinutes ? `${row.durationMinutes} min` : "Non renseignée"}</span><span><b>Téléphone</b>{row.customerPhone ?? "Non renseigné"}</span></div>{row.arrivedAt ? <p className="workspace-description">Arrivée enregistrée à {formatTime(row.arrivedAt)}{row.startedAt ? ` · travail démarré à ${formatTime(row.startedAt)}` : ""}.</p> : null}</div><div className="workspace-row-actions">{!row.arrivedAt && !["COMPLETED", "CANCELLED"].includes(row.status) ? <form action={arriveAtInterventionAction}><input type="hidden" name="interventionId" value={row.interventionId} /><button className="button primary small" type="submit">Je suis arrivé</button></form> : null}{row.arrivedAt && !row.startedAt && !["COMPLETED", "CANCELLED"].includes(row.status) ? <form action={startInterventionAction}><input type="hidden" name="interventionId" value={row.interventionId} /><button className="button primary small" type="submit">Commencer</button></form> : null}{!["COMPLETED", "CANCELLED"].includes(row.status) ? <form action={addFieldNoteAction} className="workspace-inline-form compact"><input type="hidden" name="interventionId" value={row.interventionId} /><label><span>Note terrain</span><input name="note" maxLength={4000} required placeholder="Observation factuelle" /></label><button className="button ghost small" type="submit">Ajouter la note</button></form> : null}</div></article>)}</section> : <EmptyState title="Aucune intervention aujourd’hui" description="Aucune intervention ne vous est assignée pour cette date." />}
    </>
  );
}

function ResultNotice({ result }: { result?: string }) { if (!result) return null; const good = ["arrived", "started", "note-saved", "conflict-resolved"].includes(result); const copy: Record<string, string> = { arrived: "Votre arrivée a été enregistrée.", started: "Le début de l’intervention a été enregistré.", "note-saved": "La note terrain a été synchronisée.", conflict: "La note a été conservée mais un conflit doit être vérifié.", "conflict-resolved": "Le conflit a été arbitré.", invalid: "L’action demandée est incomplète.", "not-applied": "SESIRA n’a pas pu confirmer cette action." }; return <section className="premium-inline-notice"><StatusPill tone={good ? "good" : "warning"}>{good ? "Enregistré" : "À vérifier"}</StatusPill><p>{copy[result] ?? "État inconnu."}</p></section>; }

    ...
    <div className="sesira-page--documents">
      ...
      {rows.length ? (
        <section className="workspace-stat-strip" aria-label="État des documents">
          ...
          <div><strong>{rows.filter((row) => row.extractionConfidence !== null && row.extractionConfidence < 0.5).length}</strong><span>Extraction à relire</span></div>
        </section>
      ) : null}
            ...
            <article className="workspace-row" key={row.id}>
              <div className="workspace-row-main">
                ...
                <div className="workspace-meta">
                  ...
                  <span className="document-confidence"><b>Extraction</b>{row.extractionConfidence === null ? "Non mesurée" : `${Math.round(row.extractionConfidence * 100)} %`}</span>
                </div>
                {row.extractionConfidence !== null && row.extractionConfidence < 0.5 ? <div className="workspace-gap-box"><strong>Vérification humaine requise</strong><p>La classification a une confiance faible. Le type et les champs extraits doivent être contrôlés avant validation.</p></div> : null}
              </div>

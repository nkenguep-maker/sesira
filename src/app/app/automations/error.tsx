...
export default function AutomationsError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="mx-auto max-w-2xl rounded-2xl border border-rose-400/20 bg-rose-400/10 p-8 text-center">
      ...
      <h2 className="mt-5 text-xl font-semibold">Impossible de charger les automatisations.</h2>
      <p className="mt-3 text-sm leading-6 text-rose-100/70">
        Aucun état incomplet ne sera présenté comme une activité réelle. Réessayez dans quelques instants.
      </p>
      <button
        onClick={reset}

...
export default async function RegulatoryDocumentsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <div className="sesira-page--regulatory">
      <PageHeader eyebrow="OBLIGATIONS CVC" title="Documents à préparer" description="Les éléments à préparer et à vérifier, sans confondre préparation dans SESIRA et dépôt auprès de l’organisme compétent." />
      <ResultNotice result={params.result} />

      <nav className="sesira-regulatory-jumpnav" aria-label="Sections des obligations">
        <a href="#points">Points ouverts</a>
        <a href="#cerfa">CERFA</a>
        <a href="#bilan">Bilan annuel</a>
        <a href="#attestations">Attestations</a>
        <a href="#exports">Documents préparés</a>
      </nav>
      ...
      <section id="points" className="workspace-section">
              ...
              <article className="workspace-row" key={item.id}>
                ...
                <div className="workspace-row-actions">
                  ...
                  <details className="sesira-action-drawer">
                    <summary>Clore ce point</summary>
                    <div className="sesira-action-drawer-body">
                      <form action={resolveRegulatoryAttentionAction} className="workspace-inline-form compact">
                        <input type="hidden" name="attentionId" value={item.id} />
                        <label><span>Note de résolution</span><input name="note" maxLength={2000} placeholder="Vérification effectuée, document obtenu…" /></label>
                        <button className="button primary small" type="submit">Enregistrer la clôture</button>
                      </form>
                    </div>
                  </details>
                </div>
        ...
        ) : <EmptyState title="Aucun point ouvert" description="Aucune attention réglementaire non résolue n’est enregistrée." />}
      </section>

      <section id="cerfa" className="workspace-section">
        ...
        {pendingCerfa.status === "ERROR" ? <p className="workspace-empty-line">Cette liste est momentanément indisponible.</p> : pendingCerfa.rows.length ? (
          <div className="workspace-list compact-list">
            {pendingCerfa.rows.map((row) => (
              <article className="workspace-row" key={row.interventionId}>
                <div className="workspace-row-main"><div className="workspace-row-heading"><div><span className="eyebrow">{customerNames.get(row.customerId) ?? "Client"}</span><h2>{row.title}</h2></div><StatusPill tone="warning">À préparer</StatusPill></div><p className="workspace-description">Intervention terminée{row.completedAt ? ` le ${formatDate(row.completedAt)}` : ""}. Aucun dépôt externe ne sera effectué.</p></div>
                <div className="workspace-row-actions"><form action={prepareCerfaAction}><input type="hidden" name="interventionId" value={row.interventionId} /><button className="button primary small" type="submit">Préparer le CERFA</button></form></div>
              </article>
            ))}
          </div>
        ) : <p className="workspace-empty-line">Aucune intervention n’attend actuellement cette préparation.</p>}
      </section>

      <section id="bilan" className="workspace-section">
      ...
      <section id="exports" className="workspace-section">
        ...
        {exports.length ? (
          <div className="workspace-list compact-list">
            {exports.map((row) => <article className="workspace-row" key={row.id}><div className="workspace-row-main"><div className="workspace-row-heading"><div><span className="eyebrow">{exportKind(row.kind)}</span><h2>{row.referenceYear ? `Année ${row.referenceYear}` : row.interventionId ? "Intervention liée" : "Dossier"}</h2></div><StatusPill tone={row.status === "READY" || row.status === "EXPORTED" ? "good" : row.gapCount ? "warning" : "neutral"}>{exportStatus(row.status)}</StatusPill></div><div className="workspace-meta"><span><b>Préparé</b>{formatDateTime(row.generatedAt)}</span><span><b>Informations manquantes</b>{row.gapCount}</span><span><b>Export externe</b>{row.exportedAt ? `${formatDateTime(row.exportedAt)} · ${row.exportFormat ?? "format non indiqué"}` : "Non enregistré"}</span></div></div></article>)}
          </div>
        ) : <EmptyState title="Aucun document préparé" description="Les CERFA et bilans préparés apparaîtront ici." />}
      </section>
    </div>
  );
}

create or replace function public.redact_export_json(input_value jsonb)
returns jsonb
language plpgsql
immutable
security invoker
...
begin
  ...
  if jsonb_typeof(input_value) = 'object' then
    select coalesce(jsonb_object_agg(e.key, public.redact_export_json(e.value)), '{}'::jsonb)
      into output_value
  ...
  if jsonb_typeof(input_value) = 'array' then
    select coalesce(jsonb_agg(public.redact_export_json(value)), '[]'::jsonb)
      into output_value
...
revoke all on function public.redact_export_json(jsonb) from public, anon;
grant execute on function public.redact_export_json(jsonb) to authenticated, service_role;
...
begin
  ...
  select public.redact_export_json(to_jsonb(o))
    into org_json
  ...
  loop
    execute format(
      'select coalesce(jsonb_agg(public.redact_export_json(to_jsonb(src))), ''[]''::jsonb) from public.%I src where src.organization_id = $1',
...
drop function if exists private.redact_export_json(jsonb);

...
-- This guard lives at the durable outbound-intent boundary so it protects
-- future callers in addition to the current UI-level demo guard.
...
begin
  ...
  select coalesce(
    (o.feature_flags ->> 'demo_mode') = 'true'
    or (o.config ->> 'demo_mode') = 'true'
    or (o.config ->> 'demo_data') = 'true',
    false
  )
  into target_is_demo
  ...
  if coalesce(target_is_demo, false) then
    raise exception 'record_outbound_message_intent: external actions are disabled for demo organizations'
      using errcode = '42501';
  end if;
